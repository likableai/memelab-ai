...
export default function MemeStudioPage() {
  ...
  const download = useCallback(() => {
    ...
    if (generatedFormat === 'video') {
      ...
      a.download = `memelab-meme-${Date.now()}.mp4`;
      a.href = resolvedUrl;
      a.click();
      return;
    }
    if (generatedFormat === 'gif') {
      const a = document.createElement('a');
      a.download = `memelab-meme-${Date.now()}.gif`;
      a.href = resolvedUrl;
      a.click();
      return;
    }
    ...
    if (canvas) {
      const a = document.createElement('a');
      a.download = `memelab-meme-${Date.now()}.png`;
    ...
    <AppLayout>
      <div className="container-padding mx-auto" style={{ maxWidth: 'var(--content-max-width)' }}>
        ...
        <section className="card section-spacing" style={{ padding: CARD_PADDING }}>
          <h2 className="section-title mb-4">Meme Templates</h2>
          <div
          ...
          >
            {templates.map((t) => (
              <button
                ...
                style={{
                  borderColor: selectedTemplateId === t.id ? 'var(--accent-primary)' : 'var(--border)',
                  padding: 0,
        ...
        <section className="card section-spacing" style={{ padding: CARD_PADDING, marginTop: SECTION_GAP }}>
          <h2 className="section-title mb-4">Create Your Meme</h2>
        ...
        <section className="card section-spacing" style={{ padding: CARD_PADDING, marginTop: SECTION_GAP }}>
          ...
          {!loading && generatedUrl && resolvedUrl && (
            <>
              <div className="relative inline-block rounded-xl overflow-hidden border" style={{ borderColor: 'var(--border)' }}>
